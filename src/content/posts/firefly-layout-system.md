...
description: 深入了解 Firefly 的布局系统，包括侧边栏布局（左�?双侧）和文章列表布局（列�?网格），以及自适应网格列数�?image: ./images/firefly1.avif
...
Firefly 提供了灵活的布局系统，允许您根据内容需求和个人喜好自定义博客的视觉呈现方式。布局系统主要包括**侧边栏布局**�?*文章列表布局**两个维度，它们相互配合，共同决定了页面的整体结构�?
本文将详细介�?Firefly 的各种布局模式、它们的特点、使用场景，以及不同布局组合的效果�?
---
...
当position为left或right时开启此项后，文章详情页将显示双侧边栏，主页等其他页面保持单侧边�?
适用在只想用单侧栏，但在文章详情页想用对侧栏的目录等组件的场�?:::
...
- 宽屏桌面端浏�?- 信息密集型博�?- 需要展示大量辅助内�?- 专业性强的技术博�?
...
- �?视觉冲击力强，封面图吸引眼球
- �?可以展示更多文章信息（摘要、标签等�?- �?适合图片内容丰富的博�?- �?移动端友好，单列更易阅读
- �?兼容所有侧边栏配置（单侧、双侧）
...
export const siteConfig: SiteConfig = {
  postListLayout: {
    ...
    allowSwitch: true,
    grid: {
      masonry: true,      // 开启瀑布�?      columnWidth: 320,   // 卡片最小宽�?px)，浏览器自动计算列数
    },
  },
};
```
...
- **智能排版**：自动将卡片放置到最短的列，最大化利用垂直空间�?- **消除空白**：通过绝对定位精确计算每个卡片的位置，让卡片紧贴上方卡片，消除垂直方向的空白间隙�?- **自适应列数**：瀑布流同样根�?`columnWidth` 和容器宽度动态计算列数，无需固定配置�?- **配置灵活**：您可以�?`siteConfig.ts` 中通过 `postListLayout.grid.masonry` 选项自由开启或关闭此功能�?
---
...
| 侧边栏模�?| 文章列表模式 | 推荐�?| 适用场景 |
|-----------|------------|--------|---------|
| 单侧边栏   | 列表模式    | ⭐⭐⭐⭐�?| 摄影、设计、生活类博客，强调图片和沉浸�?|
| 单侧边栏   | 网格模式    | ⭐⭐⭐⭐�?| 技术、笔记类博客，平衡阅读与检索效�?|
| 双侧边栏   | 列表模式    | ⭐⭐⭐⭐�?| 需要展示大量侧边栏信息的站�?|
| 双侧边栏   | 网格模式    | ⭐⭐⭐⭐�?| 极客风格，追求最高信息密�?|
...
1. **网格列数自动减少**：网格模式的列数�?`columnWidth` 和容器宽度自动决定，屏幕越窄列数越少�?2. **列表模式 -> 网格模式**：当屏幕宽度小于 380px（超小屏设备）时，列表模式会自动切换为网格模式，以保证卡片内容的可读性�?3. **双侧边栏 -> 单侧边栏**：当屏幕宽度小于 1280px 时，会根据`tabletSidebar`配置显示单侧边栏，隐藏其中一个侧边栏，文章目录导航会切换成浮动目录导航�?
---
