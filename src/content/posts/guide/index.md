...
这个博客模板是基�?[Astro](https://astro.build/) 构建的。对于本指南中未提及的内容，您可以在 [Astro 文档](https://docs.astro.build/) 中找到答案�?
## 文章�?Front-matter

```yaml
---
title: 我的第一篇博客文�?published: 2023-09-09
description: 这是我新 Astro 博客的第一篇文章�?image: ./cover.jpg
tags: [前端, 开发]
category: 前端开�?draft: false
---
```
...
| `image`       | 文章封面图片路径�?br/>1. �?`http://` �?`https://` 开头：使用网络图片<br/>2. �?`/` 开头：`public` 目录中的图片<br/>3. 不带任何前缀：相对于 markdown 文件的路�?|
| `tags`        | 文章标签�?                                                                                                                                                                                         |
| `category`    | 文章分类�?                                                                                                                                                                                         |
...
| `slug`        | 自定义文�?URL 路径。如果不设置，将使用文件名作�?URL�?                                                                                                                                             |
...
### 什么是 Slug�?
Slug 是文�?URL 路径的自定义部分。如果不设置 slug，系统将使用文件名作�?URL�?
### Slug 使用示例
...
1. **使用英文和连字符**：`my-awesome-post` 而不�?`my awesome post`
2. **保持简�?*：避免过长的 slug
3. **具有描述�?*：让 URL 能够反映文章内容
4. **避免特殊字符**：只使用字母、数字和连字�?5. **保持一致�?*：在整个博客中使用相似的命名模式

### 注意事项

- Slug 一旦设置并发布，建议不要随意更改，以免影响 SEO 和已存在的链�?- 如果多个文章使用相同�?slug，后面的文章会覆盖前面的
- Slug 会自动转换为小写