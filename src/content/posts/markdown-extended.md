...
您可以添加链接到 GitHub 仓库的动态卡片，在页面加载时，仓库信息会�?GitHub API 获取�?
...
Firefly 采用�?[rehype-callouts](https://github.com/lin-stephanie/rehype-callouts) 插件，支持了三种风格的提醒框主题：`GitHub`、`Obsidian` �?`VitePress`。您可以�?`src/config/siteConfig.ts` 中进行配置：
...
[VitePress](https://vitepress.dev/) 风格提供了一套现代化的、扁平的默认样式。目前仅包含�?GitHub 一致的 **5 �?* 基础类型�?
<details>
...
> [!CAUTION] CAUTION
> 对应 GitHub �?Caution�?
> [!TIP] 自定义标�?> VitePress 风格同样支持自定义标题�?```
</details>
...
您可以使�?`[grid]` �?`[/grid]` 标签将多张图片纵向并排展示。这对于展示照片画廊或对比图非常有用。系统会自动根据包裹在其中的图片数量（最多支持并排展�?张）以响应式网格进行布局�?
**自动补齐图片高度�?* 同一排中如果有高度、大小或者比例不一的图片，会像「九宫格画廊相册」一样自动撑满。较短或不协调的图片会自动使�?object-cover 进行完美中心裁剪补充视野。图片边框水平彻底对齐无缝隙，但被裁剪后，只有点击图片通过灯箱才能查看完整图片，所以建议尽量避免使用长宽比例不一致的图片在同一排中�?
