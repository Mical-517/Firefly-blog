...
这是一个展示如何编�?Markdown 文件的示例。本文档汇总了核心语法与常见扩展（GFM）�?
- [块级元素](#block-elements)
...
使用一个或多个空行分隔段落。（仅包�?*空格**�?*制表�?*的行也视为空行。）
...
Markdown 支持两种标题样式：Setext �?atx�?
#### Setext

HTML 标签：`<h1>`，`<h2>`

使用**等号 (=)** 表示 `<h1>`、使�?*短横�?(-)** 表示 `<h2>`，数量不限，作为“下划线”�?
代码�?
    This is an H1
    =============
    This is an H2
...
可选：你可以在行尾“闭合�?atx 标题。末尾的井号数量**不必�?*开头一致�?
代码�?
    # This is an H1 #
    ## This is an H2 ##
    ### This is an H3 ######
...
Markdown 使用邮件风格�?**>** 作为引用符号。若手动换行并在每行前加 >，显示效果最佳�?
代码�?
...
Markdown 允许“偷懒”：在一个硬换行段落中，只在第一行前�?> 即可�?
代码�?
    > This is a blockquote with two paragraphs. Lorem ipsum dolor sit amet,
    consectetuer adipiscing elit. Aliquam hendrerit mi posuere lectus.
    Vestibulum enim wisi, viverra nec, fringilla in, laoreet vitae, risus.
...
HTML 标签：`<hr />`
一行中放置**三个或以上的短横�?(-)、星�?(\*) 或下划线 (\_)**。符号之间允许有空格�?
代码�?
    * * *
    ***
    *****
...
这是扩展语法�?
�?*竖线 (|)** 分隔列，�?*短横�?(-)** 分隔表头，使�?*冒号 (:)** 指定对齐方式�?
两侧�?*竖线 (|)** 与对齐可选。用于表头分隔时，每列至少需�?**3 个短横线**�?
代码�?
```
| Left | Center | Right |
|:-----|:------:|------:|
...
说明�?
- 方括号中包含链接标识�?*不区分大小写**，可在左侧缩进最多三格空格）�?- 随后是冒号；
- 再跟一个或多个空格（或 tab）；
- 然后是链�?URL�?- URL 可选地用尖括号包裹�?- 可选地跟随标题属性，用引号或圆括号包裹�?
以下三种定义等价�?
代码�?
...
Markdown 使用 **星号 (\*)** �?**下划�?(\_)** 表示强调�?*一个分隔符**对应 `<em>`�?*两个分隔�?*对应 `<strong>`�?
代码�?
    *single asterisks*
...
若行内代码中需要包含反引号字符，可使用**多重反引�?*作为定界符：
...
### 反斜杠转�?
Markdown 允许使用反斜杠来转义那些本用�?Markdown 语法的特殊字符，使其按字面显示�?
代码�?
    \*literal asterisks\*
...
对于 Markdown 语法未覆盖的标记，直接使用原�?HTML 即可。无需特别声明�?Markdown 切换�?HTML，直接写标签就行�?
代码�?
...
请注意：�?*块级 HTML 标签**内不会处�?Markdown 语法�?
与块级标签不同，�?*行内级标�?*内会处理 Markdown 语法�?
代码�?
    <span>**Work**</span>
